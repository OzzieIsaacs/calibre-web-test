...
class SMTP(asyncio.StreamReaderProtocol):
    ...
    @syntax('AUTH arg', when='enable_Auth')
    async def smtp_AUTH(self, arg):
        if not arg:
            await self.push('500 Syntax error, command unrecognized')
            return
        if self.seen_greeting == False:
            await self.push('503 Bad sequence of commands')
            return
        ...
        if 'PLAIN' in arg or self.auth_type=='PLAIN':
            if arg == 'PLAIN':
                ...
                if hasattr(self.event_handler, 'handle_AUTH'):
                    status = await self._call_handler_hook('AUTH', self.username, self.password)
                    if status == 235:
                        self.authenticated = True
                        await self.push('235 Authentication successful.')
                    else:
                        await self.push('454 Temporary authentication failure.')
                        self._set_rset_state()
                        self._handler_coroutine.cancel()
                        self.transport.close()
                        return
            ...
            if len(split_args) == 2:
                ...
                await self.push('334 ' + encode_b64('Username'))
            else:
                await self.push('334 ' + encode_b64('Username'))
        elif not self.username:
            self.username = decode_b64(arg)
            await self.push('334 ' + encode_b64('Password'))
        else:
            self.authenticating = False
            self.password = decode_b64(arg)
            if hasattr(self.event_handler, 'handle_AUTH'):
                status = await self._call_handler_hook('AUTH', self.username, self.password)
                if status and status != 454:
                    self.authenticated = True
                    await self.push('235 Authentication successful.')
                    return
                else:
