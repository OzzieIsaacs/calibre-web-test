...
class TestEditAdditionalBooks(TestCase, ui_class):
    ...
    def test_upload_edit_role(self):
        ...
        result = r.post('http://127.0.0.1:8083/admin/book/7', files=files_format, data=values)
        self.assertEqual(403, result.status_code)
        upload_file.close()
        cover_file = open(os.path.join(base_path, 'files', 'cover.jpg'), 'rb')
        files_cover = {'btn-upload-cover': cover_file}
        result = r.post('http://127.0.0.1:8083/admin/book/7', files=files_cover, data=values)
        self.assertEqual(403, result.status_code)
        cover_file.close()
        values['cover_url'] = "/home/user/kurt.jpg"
        result = r.post('http://127.0.0.1:8083/admin/book/7', data=values)
        self.assertEqual(403, result.status_code)
        r.close()
        ...
        payload = {'username': 'user0', 'password': '1234', 'submit': "", 'next': "/", "remember_me": "on"}
        r.post('http://127.0.0.1:8083/login', data=payload)
        result = r.get('http://127.0.0.1:8083/admin/book/7')
        self.assertEqual(403, result.status_code)
        r.close()
